...
//             columns={[
//                     { accessor: 'id', title: 'Select', sortable: false, render: (record) => (
...
const Assign = () => {
    ...
    useEffect(() => {
        ...
        const sortChanged = combinedRef.current.prevSortStatus.columnAccessor !== sortStatus.columnAccessor || combinedRef.current.prevSortStatus.direction !== sortStatus.direction;
    ...
    // const fetchLeads = () => {
    //     dispatch(newleads({  page: currentPage,  perPage, sortField: sortStatus.columnAccessor, sortOrder: sortStatus.direction }));
    // };

    // useEffect(() => {
    //     fetchLeads();
    // }, [currentPage, perPage, sortStatus]);
        ...
        <div>
            ...
            <div className="datatables mt-6">
                {loading ? ( 
                    ...
                    <Table title="New Leads" columns={columns} rows={tableData} totalRecords={total || 0} currentPage={currentPage} recordsPerPage={perPage} onPageChange={handlePageChange} onRecordsPerPageChange={handlePerPageChange} onSortChange={handleSortChange} sortStatus={sortStatus} isLoading={loading} />
                ) : (
                    <div className="panel text-center text-primary-500 mt-4">
                        <span className='badge bg-secondary'> Sorry, I am unable to retrieve data. Please check your API . </span>
                    </div>
